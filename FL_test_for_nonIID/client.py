...
class RetinopathyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if original_gradients and noisy_gradients:
            # Ensure we're comparing corresponding layers
            for i in range(min(len(original_gradients), len(noisy_gradients))):
                if original_gradients[i] is not None and noisy_gradients[i] is not None:
                    # Use the last trainable layer (typically contains most information)
                    if i == len(original_gradients) - 1:
                        plot_gradient_distribution(
                            original_gradients[i],
                            noisy_gradients[i],
                            self.client_id,
                            self.current_round
                        )

            # Visualize gradient norm effects to demonstrate clipping
            ...
            if original_norms and len(original_norms) > 0:
                plot_gradient_norm_distribution(
                    original_norms,
                    clipped_norms,

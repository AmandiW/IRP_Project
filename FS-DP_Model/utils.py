...
class DenseNetCBAM(nn.Module):
    ...
    def __init__(self, model_name='densenet121', num_classes=2, pretrained=True):
        ...
        # Using GroupNorm for ResNet layers as that's the main model used for Evaluation, DenseNet was just an extension
...
def train_with_feature_specific_dp(model, train_loader, optimizer, criterion, device, dp_params, logger, epochs=1,
    ...
    3. Privacy guarantees:
       - Overall per round epsilon remains the same as standard DP-SGD
    ...
    # Calculate privacy metrics using the RDP accounting
...
def calculate_theoretical_leak_probability(epsilon):
    ...
    """
    # Using the standard DP definition
    # We can derive a simple measure of distinguishability
...
def plot_client_vs_global_accuracy_per_round(rounds_data):
    ...
    if 'global_accuracy' in rounds_data.columns:
        ...
        # If global accuracy isn't available, we can use the average accuracy as an approximation
...
def visualize_privacy_preservation_with_reconstruction(model, data_loader, device, noise_multiplier=1.0,
    ...
    # Helper function to safely convert tensor to numpy image
    def tensor_to_numpy_image(tensor):
        """Safely convert tensor to numpy image regardless of dimension."""
        tensor = tensor.cpu()
        ...
        if tensor.dim() == 3:  # [C,H,W]
            ...
            tensor = tensor.unsqueeze(2)  # Add channel dimension
    ...
    for i in range(num_images):
        ...
        noisy_img_np = tensor_to_numpy_image(noisy_img)

        # Reconstructed image (adversary's attempt)
        recon_img = reconstructed_images[i][0].cpu()  # Handle the batch dimension
        recon_img = recon_img * std.cpu() + mean.cpu()
        recon_img_np = tensor_to_numpy_image(recon_img)

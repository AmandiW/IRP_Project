...
class RetinopathyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # Demonstrate gradient inversion protection with proper error handling
        try:
            self.logger.info(f"Client {self.client_id} - Attempting to create gradient inversion demonstration")
            ...
            sample_image = sample_batch[0][0]  # Get the first image from batch
            self.logger.info(f"Client {self.client_id} - Sample image shape: {sample_image.shape}")

            # Call the demonstration function with detailed logging
            result = demonstrate_gradient_inversion(
                model=self.model,
            ...
            if result and len(result) == 3:
                _, _, filepath = result
                self.logger.info(
                    f"Client {self.client_id} - Created gradient inversion protection demonstration at: {filepath}")
            else:
                self.logger.warning(
                    f"Client {self.client_id} - Gradient inversion demonstration returned unexpected result: {result}")

        except Exception as e:
            self.logger.error(f"Error creating gradient inversion demonstration: {e}", exc_info=True)

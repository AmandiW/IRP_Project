...
# ========================= Feature-Specific DP-SGD with CBAM =========================
def train_with_feature_specific_dp(model, train_loader, optimizer, criterion, device, dp_params, logger, epochs=1,
    ...
    # Training loop with improved DP-SGD using microbatching and feature-specific privacy
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            for sample_idx in range(current_batch_size):
                ...
                # Save attention maps for feature-specific privacy
                if feature_specific and hasattr(model,
                                                'get_private_features') and epoch == epochs - 1 and batch_idx % 20 == 0 and sample_idx == 0:
                    attention_maps_history.append(model.get_private_features())

                # Get trainable parameters
                trainable_params = [p for p in model.parameters() if p.requires_grad]

                # Calculate gradient norm and store gradients
                sample_grads = []
                grad_norm = 0.0
            ...
            # Phase 2: Clip and aggregate gradients with feature-specific sensitivity
            optimizer.zero_grad()
            ...
            if feature_specific and hasattr(model, 'get_private_features'):
                ...
                feature_sensitivity = model.get_private_features()

                # If no sensitivity scores, fall back to standard DP-SGD
                if not feature_sensitivity:
                    feature_specific = False
                    logger.info("No feature sensitivity scores available, using standard DP-SGD")

            # Add clipped gradients with feature-specific noise
            for sample_idx in range(current_batch_size):
                grad_norm = per_sample_norms[sample_idx]
                # Avoid division by zero
                scale = min(1.0, max_grad_norm / (grad_norm + 1e-12))

                # Add scaled gradients for each parameter
                for param_idx, param in enumerate(trainable_params):
                    if param_idx < len(per_sample_grads[sample_idx]):
                        sample_grad = per_sample_grads[sample_idx][param_idx]
                        # Ensure tensor shapes match
                        if param.grad.shape == sample_grad.shape:
                            param.grad += sample_grad * scale / current_batch_size
                        else:
                            logger.warning(
                                f"Shape mismatch: param.grad {param.grad.shape} vs sample_grad {sample_grad.shape}")

            # Phase 3: Add noise to accumulated gradients - feature-specific if available
            if feature_specific and feature_sensitivity:
                # Get sensitivity scores for all layers
                sensitivity_scores = list(feature_sensitivity.values())

                # Normalize sensitivity scores
                if len(sensitivity_scores) > 0:
                    max_sensitivity = max(sensitivity_scores)
                    min_sensitivity = min(sensitivity_scores)

                    # Normalize to [0.5, 1.5] range to avoid too much or too little noise
                    normalized_sensitivity = {
                        layer: 0.5 + ((score - min_sensitivity) / (max_sensitivity - min_sensitivity + 1e-8))
                        for layer, score in feature_sensitivity.items()
                    }

                    # Apply different noise levels based on feature sensitivity
                    param_idx = 0
                    for name, module in model.named_modules():
                        # Check if this module's parameters need feature-specific privacy
                        scale_factor = 1.0
                        for layer_name in feature_sensitivity.keys():
                            if layer_name in name:
                                scale_factor = normalized_sensitivity[layer_name]
                                break

                        # Add noise to module parameters
                        for param in module.parameters():
                            if param.requires_grad and param.grad is not None:
                                # Scale noise by sensitivity
                                noise = torch.randn_like(
                                    param.grad) * noise_multiplier * max_grad_norm * scale_factor / math.sqrt(
                                    current_batch_size)
                                param.grad += noise
                                param_idx += 1
                else:
                    # Fall back to standard noise addition if no sensitivity scores
                    for param in trainable_params:
                        if param.grad is not None:
                            noise = torch.randn_like(param.grad) * noise_multiplier * max_grad_norm / math.sqrt(
                                current_batch_size)
                            param.grad += noise
            else:
                # Standard noise addition (no feature-specific privacy)
                for param in trainable_params:
                    if param.grad is not None:
                        noise = torch.randn_like(param.grad) * noise_multiplier * max_grad_norm / math.sqrt(
                            current_batch_size)
...
def plot_feature_specific_privacy_impact(attention_maps_history, client_id, round_num):
    ...
    os.makedirs("./visualizations/feature_specific_privacy", exist_ok=True)
    ...
    plt.tight_layout()
    filepath = f"./visualizations/feature_specific_privacy/client_{client_id}_feature_sensitivity_round_{round_num}.png"
    plt.savefig(filepath)
    plt.close()
...
def visualize_model_reconstruction_threat(model, device, batch_size=8, grid_size=4):
    """
    Visualize the potential threat of model inversion attacks.

    Args:
        model (nn.Module): Trained model
        device (torch.device): Device to use
        batch_size (int): Batch size for generated images
    ...
    # Generate random noise as starting point for optimization
    grid_size = min(grid_size, 8)  # Cap grid size for reasonable visualization
    num_samples = grid_size * grid_size

    # Random target classes (binary classification: 0 or 1)
    target_classes = torch.randint(0, 2, (num_samples,)).to(device)

    # Create random noise images (starting points)
    random_images = torch.randn(num_samples, 3, 224, 224).to(device)
    random_images.requires_grad_(True)

    # Simple optimization to maximize class probability (simulating model inversion)
    optimizer = torch.optim.Adam([random_images], lr=0.01)
    ...
    for i in range(num_iterations):
        ...
        # Calculate loss (negative log probability of target class)
        probs = torch.nn.functional.softmax(outputs, dim=1)
        target_probs = probs[range(num_samples), target_classes]
        ...
        # Add total variation regularization (for smoother images)
        if i % 10 == 0:
            # Normalize images to be between 0 and 1
            with torch.no_grad():
                random_images.data = torch.clamp(random_images.data, -2.5, 2.5)
    ...
    with torch.no_grad():
        ...
        images = random_images * std + mean
        images = torch.clamp(images, 0, 1)

    # Create grid visualization
    grid_images = images.detach().cpu()
    grid = torchvision.utils.make_grid(grid_images[:grid_size * grid_size], nrow=grid_size)
    grid = grid.permute(1, 2, 0).numpy()

    # Plot images
    plt.figure(figsize=(10, 10))
    plt.imshow(grid)
    plt.title("Model Inversion Attack Simulation\n(This shows what an attacker might reconstruct)", fontsize=14)
    plt.axis('off')

    filepath = "./visualizations/privacy_analysis/model_inversion_threat.png"
    plt.savefig(filepath)
    plt.close()
...
def plot_privacy_amplification_by_sampling(sample_rates, noise_multiplier=1.0, steps=1000, delta=1e-5):
    """
    Visualize privacy amplification by sampling effect.

    Args:
        sample_rates (list): List of sampling rates to compare
        noise_multiplier (float): Fixed noise multiplier value
        steps (int): Number of SGD steps
        delta (float): Delta privacy parameter
    ...
    # Calculate epsilon for different sampling rates
    epsilons = []
    for rate in sample_rates:
        eps = compute_dp_sgd_privacy_budget(noise_multiplier, rate, steps, delta)
        epsilons.append(eps)

    plt.figure(figsize=(10, 6))

    # Plot epsilon vs sampling rate
    plt.plot(sample_rates, epsilons, 'ro-', linewidth=2, markersize=8)

    plt.title(f'Privacy Amplification by Sampling (σ={noise_multiplier}, steps={steps})', fontsize=14)
    plt.xlabel('Sampling Rate', fontsize=12)
    plt.ylabel('Privacy Budget (ε)', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.7)

    # Add annotations
    for i, (rate, eps) in enumerate(zip(sample_rates, epsilons)):
        if i % (len(sample_rates) // 5 + 1) == 0:
            plt.annotate(f"ε={eps:.2f}",
                         (rate, eps),
                         xytext=(5, 5),
                         textcoords="offset points")

    filepath = "./visualizations/privacy_analysis/privacy_amplification_by_sampling.png"
    plt.savefig(filepath)
    plt.close()

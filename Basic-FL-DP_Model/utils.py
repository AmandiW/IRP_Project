...
class SimpleModel(nn.Module):
    ...
    def __init__(self, num_classes=2):
        ...
        # MODIFIED: Make the final layer and the last layer3 and layer4 blocks trainable
        # This allows more expressive power while still keeping the model efficient for DP
        for name, param in self.backbone.named_parameters():
            if any(layer_name in name for layer_name in ['layer3.1', 'layer4', 'fc']):
...
def create_model(num_classes=2):
    ...
    # More robust model definition to ensure consistent architecture
    class EnhancedSimpleModel(nn.Module):
        def __init__(self, num_classes=2):
            ...
            # Modify the final layer for better classification
            num_features = self.backbone.fc.in_features
            ...
            # Selectively enable training for specific layers
            # These will be the same layers that are set to trainable in client.set_parameters
    ...
    model = EnhancedSimpleModel(num_classes)

    # Log trainable parameter stats
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total_params = sum(p.numel() for p in model.parameters())
    logger = logging.getLogger("ModelCreation")
    logger.info(f"Created model with {trainable_params} trainable parameters out of {total_params} total parameters")
...
def train_with_dp(model, train_loader, optimizer, criterion, device, dp_params, logger, epochs=1,
    ...
    # MODIFIED: More robust tracking of trainable parameters and their indices
    trainable_params = []
    param_indices = {}
    for i, p in enumerate(model.parameters()):
        if p.requires_grad:
            trainable_params.append(p)
            param_indices[i] = len(trainable_params) - 1
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            for sample_idx in range(current_batch_size):
                ...
                # MODIFIED: Only collect gradients for trainable parameters
                sample_grads = []
                grad_norm = 0.0

                # Calculate gradient norm and store gradients only for trainable parameters
            ...
            # MODIFIED: Clip and aggregate gradients more carefully
            # Initialize gradients for all trainable parameters
            for idx, param in enumerate(trainable_params):
                param.grad = torch.zeros_like(param.data)

            # Add clipped gradients
            for sample_idx in range(current_batch_size):
            ...
            # Phase 3: Add noise to accumulated gradients as in Abadi paper
            for param in trainable_params:
                if param.grad is not None:
            ...
            with torch.no_grad():
                ...
                # Add FedProx regularization to loss calculation for reporting as in Li paper
                if using_fedprox:
                    proximal_term = 0.0
                    for local_param, global_param in zip(model.parameters(), global_model.parameters()):
                        if local_param.requires_grad:

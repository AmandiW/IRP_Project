...
from utils import (
    ...
    visualize_attack_risk_reduction, FocalLoss
...
class RetinopathyClient(fl.client.NumPyClient):
    ...
    def __init__(self, client_id, train_loader, test_loader, train_size, test_size, dp_params=None):
        ...
        self.dp_params = dp_params or {
            "noise_multiplier": 0.8,  # MODIFIED: Lower noise multiplier for better learning
            "max_grad_norm": 1.0,  # Lower = more privacy, less accuracy
            "delta": 1e-5,  # Fixed delta value for privacy accounting
            "epochs": 3  # MODIFIED: Increased epochs per round
        }
        ...
        self.proximal_mu = 0.01  # Default value, will be updated from config

        # Verify that multiple layers are trainable (not just the final layer)
    ...
    def set_parameters(self, parameters):
        ...
        # FIXED: More carefully specify which layers should be trainable
        # First set all parameters to non-trainable
        for param in self.model.parameters():
            param.requires_grad = False

        # Then selectively enable training for specific layers
        for name, param in self.model.named_parameters():
            if 'backbone.fc' in name:
                # Always make the final fully connected layer trainable
                param.requires_grad = True
            elif 'backbone.layer4' in name:
                # Make the entire layer4 trainable
                param.requires_grad = True
            elif 'backbone.layer3.1' in name:
        ...
        trainable_params = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        total_params = sum(p.numel() for p in self.model.parameters())
        trainable_layers = [name for name, param in self.model.named_parameters() if param.requires_grad]
    ...
    def fit(self, parameters, config):
        ...
        epochs = self.dp_params.get("epochs", 3)  # Default to 3 epochs for better learning
        ...
        os.makedirs("./visualizations/gradient_reconstruction", exist_ok=True)
        os.makedirs("./visualizations/privacy_analysis", exist_ok=True)
...
def start_client(client_id, dp_params=None):
    ...
    if dp_params is None:
        dp_params = {
            "noise_multiplier": 0.8,  # MODIFIED: Lower noise for better learning
            "max_grad_norm": 1.0,
            "delta": 1e-5,
            "epochs": 3  # MODIFIED: Increased epochs
        }
    ...
    try:
        ...
        client_data = load_data(
            img_dir="E:/IRP_dataset_new/IRP_Final_Images",
            labels_path="E:/IRP_dataset_new/IRP_Final_Labels.csv",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        epochs = int(sys.argv[4]) if len(sys.argv) > 4 else 3  # MODIFIED: Default to 3 epochs
        dp_params = {
